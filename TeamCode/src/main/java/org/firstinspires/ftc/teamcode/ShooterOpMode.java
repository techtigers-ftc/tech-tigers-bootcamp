        ...
        while (opModeIsActive()) {
            if (robotStatus.equals("READY")) {
                ...
                if (gamepad1.b) {
                    ...
                    timer.reset(); // Reset the timer to track how long the servo has been moving for
            ...
            } else if (robotStatus.equals("GRIPPING")) {
                if (timer.seconds() > 0.2) {
                    // It has been enough time since we started gripping
                    if (gamepad1.a) {
                        timer.reset(); // Reset the timer when starting to drive
                        driveSubsystem.drive(0.5, 0, 0); // Start driving forward at half speed
                        robotStatus = "DRIVING"; // Mark that we are now driving
                    }
                }
            } else if (robotStatus.equals("DRIVING")) {
                // Check if 0.5 seconds have passed, otherwise do nothing as you continue driving
                ...
                // Before we do anything else, we need to wait for the shooting to finish
                if (timer.seconds() > 0.2) {
